...
def paiement_resultat(request, client_ref):  # SUM UP WIDGET REDIRIGE ICI APRÈS PAIEMENT
    ...
    while tries < 5 :
        if paiement.checkout_status == "PAID":
            ...
            make_pdf(f"bon_de_reservation_{client_ref}.pdf","template_bon_reservation.html", context_client,"reservations/output/","reservations/static/css/style_bon.css")
...
def download_pdf(request, client_ref):
    pdf_path = os.path.join(settings.MEDIA_ROOT, "reservations/output", f"bon_de_reservation_{client_ref}.pdf")
